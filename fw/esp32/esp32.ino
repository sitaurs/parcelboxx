...
void lockPulseMs(uint32_t ms){
  ...
  // NON-BLOCKING pulse - don't let flash LED or other operations block solenoid timing
  unsigned long startPulse = millis();
  while (millis() - startPulse < ms) {
    if (stopAll || stopLock) {
      relayWrite(PIN_REL1, false);
      return;
    }
    // Keep MQTT alive but don't use breatheDelayCancelable (causes timing issues with flash LED)
    if (mqtt.connected()) mqtt.loop();
    yield(); // Allow ESP32 to handle WiFi/system tasks
  }
